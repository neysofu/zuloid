...
test_960(void)
{
	for (size_t i = 0; i < NUMBER_OF_TESTS; i++) {
		...
		Bitboard bishops_on_white_squares = position.bb[PIECE_TYPE_BISHOP] & 0x5555555555555555;
		Bitboard bishops_on_black_squares = position.bb[PIECE_TYPE_BISHOP] & 0xaaaaaaaaaaaaaaaa;
		Bitboard occupancy = position.bb[COLOR_WHITE] | position.bb[COLOR_BLACK];
		Bitboard expected_occupancy = POSITION_INIT.bb[COLOR_WHITE] | POSITION_INIT.bb[COLOR_BLACK];
