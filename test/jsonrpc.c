...
test_jsonrpc_error_response_to_unparsable_request(void)
{
	...
	char *response = engine_send_request(engine, "spam");
...
{
	struct Engine *engine = engine_new();
	char *response = engine_send_request(engine, "{}");
	TEST_ASSERT(response && strstr(response, PROPERTY_NAME_ERROR));
	free(response);
	free(engine);
}
...
{
	struct Engine *engine = engine_new();
	char *response = engine_send_request(engine, "{\"id\":42}");
...
{
	struct Engine *engine = engine_new();
	TEST_ASSERT_NULL(engine_send_request(engine, "{\"method\":\"abcxyz\",\"params\":[]}"));
	free(engine);
}
