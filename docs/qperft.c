...
int times[MAXTIM];
char *(names[MAXTIM]) = { "pintest", "contact", "castle", "ep-capt", "capture", "pawns",
	                      "leapers", "sliders", "filter", "king   ", "do    ",  "undo   ",
	                      "recapt.", "",        "",       "",        "",        "total  " };
#else
#define TIME(A)
#endif
...
char *Zob[2 * NPCE];
unsigned char pc[NPCE * 4 + 1], /* piece list, equivalenced with various piece info  */
  ...
  array[] = { 12, 1, 14, 11, 0, 15, 2, 13, 5, 3, 4, 6, 7, 4, 3, 5 },
  capts[] = { 0, C_PPAWN, C_MPAWN, C_KNIGHT, C_BISHOP, C_ROOK, C_QUEEN, C_KING };
char noUnder = 0, queen_dir[] = { 1, -1, 16, -16, 15, -15, 17, -17 },
...
unsigned long long int HashKey = 8729767686LL, HighKey = 1234567890LL, count, epcnt, xcnt,
                       ckcnt, cascnt, promcnt, nodecount; /* stats */
FILE *f;
clock_t ttt[30];
...
{ /* fill 0x88-style attack tables, with capture codes and vectors */
	...
	for (i = 0; i < 8; i++) { /* in all directions */
...
checker(int col)
{
	...
	for (i = 0; i < 8; i++) {
		...
		if ((piece & COLOR) == (col ^ COLOR)) {
			if (code[piece - WHITE] & capt_code[-v])
				return x;
		}
		v = knight_rose[i];
		x = pos[col - WHITE] + v;
...
ReadFEN(char *FEN)
{
	...
	p = FEN;
	for (row = 7; row >= 0; row--) { /* read one row of the FEN */
		file = 0;
		do {
			c = *p++;
			...
			if (c >= '1' && c <= '8') {
				...
				switch (c) {
					case 'K':
						if (pos[col - WHITE] > 0)
							return -1; /* two kings illegal */
						Piece = KING;
						nr = col - WHITE;
						if (0x20 * row == 7 * (col - WHITE) && file == 4)
							cc = (col | col >> 2 | col >> 4);

						break;
					case 'R':
						Piece--;
						if (0x20 * row == 7 * (col - WHITE)) { /* only Rooks on a1, h1, a8,
							                                      h8 get castling spoiler */
							if (file == 0)
								cc = col >> 2;
							if (file == 7)
								cc = col >> 4;
					...
					case 'Q':
						Piece += 2;
					case 'B':
						if (--FirstSlider[col] <= LastKnight[col])
							return (-2);
						nr = FirstSlider[col];
						break;
					case 'P':
						if (--FirstPawn[col] < col - WHITE + 16)
							return (-4);
						nr = FirstPawn[col];
						Piece = col >> 5;
						break;
		...
		if (file == 8) {
			c = *p++;
			if (row > 0 && c != '/')
				return (-10); /* bad format */
			if (row == 0 && c != ' ')
				return -11;
		}
	}
	if (pos[0] == 0 || pos[WHITE] == 0)
		return -5; /* missing king */
...
void
move_gen(int color, int lastply, int d)
{
	...
	for (p = FirstSlider[COLOR - color]; p < COLOR - WHITE + 16 - color;
	     p++) {     /* run through enemy slider list */
		j = pos[p]; /* enemy slider */
		if (j == 0)
			continue;                                 /* currently captured */
		if (capt_code[j - k] & code[p] & C_DISTANT) { /* slider aimed at our king */
			v = delta_vec[j - k];
			x = k; /* trace ray from our King */
			while ((m = board[x += v]) == DUMMY)
				;
			if (x == j) { /* distant check detected         */
				in_check += 2;
				checker = j;
				check_dir = v;
			} else if (m & color) { /* first on ray from King is ours */
				y = x;
				while (board[y += v] == DUMMY)
					;
				if (y == j) { /* our piece at x is pinned!  */
					...
					if (kind[m] < 3) { /* flag promotions */
					...
					} else if (code[m] & capt_code[j - k] &
					           C_DISTANT) { /* slider moves along pin ray */
						y = x;
						...
						y = x;
						while ((y -= v) != k) { /* moves towards King     */
							PUSH(z, y)
						}
					}
				}
	...
	if (in_check) { /* purge moves with pinned pieces if in check   */
		msp = first_move;

		if (in_check > 2)
			goto King_Moves; /* double check */
		if (checker == ep_flag) {
			ep1 = msp;
			goto ep_Captures;
		}
	...
	ep1 = msp;
	if (!(color & CasRights)) {
		if (!(board[k + 1] ^ DUMMY | board[k + 2] ^ DUMMY | CasRights & color >> 4))
...
ep_Captures:
	...
	if ((board[x] & mask) == mask)
		PUSH(x << 8, ep_flag ^ 0x10 | 0xA0000000);
...
Regular_Moves:
	if (in_check & 1) {
		...
		if (!((prank ^ y) & 0xF0))
			Promo++, z |= 0xA1000000;
		if ((board[y + 1] & m) == m && pos[board[y + 1] - WHITE])
			PUSH(y + 1 << 8, z)
		if ((board[y - 1] & m) == m && pos[board[y - 1] - WHITE])
			PUSH(y - 1 << 8, z)
	...
	/* Basic move generator for generating all moves */
	{
		...
		for (p = color - WHITE + 15; p >= FirstSlider[color]; p--) {
			...
			if (kind[p] - 3 & 2) { /* scan 4 rook rays for R and Q */
				y = x;
				do {
					if (!((h = board[y += 1]) & color))
						PUSH(z, y)
				} while (!(h & COLOR));
				y = x;
				do {
			...
			if (kind[p] - 3 & 1) { /* scan 4 bishop rays for B and Q */
				y = x;
				do {
					if (!((h = board[y += 15]) & color))
						PUSH(z, y)
				} while (!(h & COLOR));
				y = x;
				do {
		...
		if (in_check)
			for (i = first_move; i < msp; i++) /* go through all moves */
			{
				if (delta_vec[(j = stack[i] & 0xFF) - k] != check_dir)
					stack[i--] = stack[--msp];
				else { /* on check ray, could block or capture checker*/
					x = k;
					do {
						x += check_dir;
						if (x == j)
...
King_Moves:
	...
	while (psp > 0) { /* pop pinned piece and link in old place it remembers*/
		m = pstack[--psp];
		pos[m] = ppos[psp];
	}
}
...
#if 1
int
move_count(int color, int lastply, int d)
{
	int i, j, k, p, v, x, z, y, r, m, h;
	int savsp, mask, new, forward, rank, prank, xcolor, ep_flag;
	int in_check = 0, checker = -1, check_dir = 20;
	...
	if (in_check) { /* purge moves with pinned pieces if in check   */
		...
		if (in_check > 2)
			goto King_Moves2; /* double check */
		if (checker == ep_flag)
			goto ep_Captures_in_Check;
	...
	/* Basic move generator for generating all moves */
	{
		...
		for (p = color - WHITE + 15; p >= FirstSlider[color]; p--) {
			...
			if (kind[p] - 3 & 2) { /* scan 4 rook rays for R and Q */
				register int h, y;
				y = x;
				while ((h = board[y += 1]) == DUMMY)
					myCount++;
				myCount += !(board[y] & color);
				y = x;
				while ((h = board[y -= 1]) == DUMMY)
					myCount++;
				myCount += !(board[y] & color);
				y = x;
				while ((h = board[y += 16]) == DUMMY)
					myCount++;
				myCount += !(board[y] & color);
				y = x;
				while ((h = board[y -= 16]) == DUMMY)
...
void
leaf_perft(int color, int lastply, int depth, int d)
{ /* recursive perft, with in-lined make/unmake */
	int i, j, k, m, x, y, v, h, p, oldpiece, store, myCount;
	int first_move, piece, victim, pred, succ, from, to, capt, mode, pcnr, vicnr,
	  in_check = 0, checker = -1, check_dir = 20, legal;
	...
	legal = 0;
	count += move_count(
	  color, lastply, d); /* bulk count, but generate moves that need legality checking */
	...
	for (i = first_move; i < msp; i++) /* go through all moves */
	{
		...
		if (mode) { /* e.p. or castling, usually skipped  */
			/* e.p.:shift capture square one rank */
			if (mode == 0xA0)
				capt ^= 0x10;
			else if (mode == 0xA1) { /* Promotion. Shuffle piece list :( */
				oldpiece = piece;
				pos[piece - WHITE] = 0;
		...
		if ((unsigned int)stack[i] >= 0xA1000000) /* was castling or prom */
		{
			if (mode == 0xA1) {
				if (noUnder) {
					...
					pos[piece - WHITE] = from;
					board[from] = piece;
				} else if (--kind[piece - WHITE] >= KNIGHT) {
					if (kind[piece - WHITE] ==
					    KNIGHT) { /* Knight must be put in Knight list */
						FirstSlider[color]++;
						piece = ++LastKnight[color] + WHITE;
						pos[piece - WHITE] = from;
						kind[piece - WHITE] = KNIGHT;
						Zob[piece - WHITE] =
						  Keys + 128 * KNIGHT + (color & BLACK) / 8 - 0x22;
					} else
						Zob[piece - WHITE] -= 128;
					code[piece - WHITE] = capts[kind[piece - WHITE]];
					goto minor; /* try minor promotion */
				} else {        /* All promotions tried, demote to Pawn again */
...
void
perft(int color, int lastply, int depth, int d)
{ /* recursive perft, with in-lined make/unmake */
	...
	for (i = flag ? ep1 : first_move; i < msp; i++) /* go through all moves */
	{
		...
		if (depth == 1) {
			...
			HashKey ^= Zobrist(piece, from) /* key update for normal move */
			           ^ Zobrist(piece, to) ^ Zobrist(victim, capt);
			HighKey ^= Zobrist(piece, from + 8) /* key update for normal move */
			           ^ Zobrist(piece, to + 8) ^ Zobrist(victim, capt + 8);
			...
			if (depth > 2) {
				...
				if (depth > 7) { // the count will not fit in 32 bits
					if (depth > 9) {
						...
						Bucket = Hash + (Index + ((int)HashKey) & HashSection) +
						         (depth - 3) * HashSection;
					if (Bucket->l.Signature1 == HighKey &&
					    Bucket->l.Signature2 == HashKey) {
						count += Bucket->l.longCount;
						accept[depth]++;
						goto quick;
					}
					reject[depth]++;
				...
				Bucket =
				  Hash + (Index + ((int)HashKey) & HashSection) + (depth - 3) * HashSection;
			} else
				Bucket = ExtraHash + (Index + ((int)HashKey) & XSIZE - 1);

			store = (HashKey >> 32) & 1;
			if (Bucket->s.Signature[store] == HighKey &&
			    (Bucket->s.Extension[store] ^ (HashKey >> 32)) < 2) {
				count += Bucket->s.Count[store];
				accept[depth]++;
				Bucket->s.Extension[store] &= ~1;
				Bucket->s.Extension[store ^ 1] |= 1;
				goto quick;
			}
			if (Bucket->s.Signature[store ^ 1] == HighKey &&
			    (Bucket->s.Extension[store ^ 1] ^ (HashKey >> 32)) < 2) {
				count += Bucket->s.Count[store ^ 1];
				accept[depth]++;
				Bucket->s.Extension[store ^ 1] &= ~1;
				Bucket->s.Extension[store] |= 1;
				goto quick;
			...
			reject[depth]++; // miss;
			if (Bucket->s.Extension[store ^ 1] & 1)
				store ^= 1;
		}
	minor:
		/* perform move, in piece list and on board */
		...
		if ((piece != color && mode != 0xA0) ||
			...
			if (depth == 2)
				leaf_perft(COLOR - color, stack[i], depth - 1, d + 1);
			else
...
main(int argc, char **argv)
{
	...
	if (argc > 1 && argv[1][0] == 'H' && sscanf(argv[1] + 1, "%d", &HashSize) == 1) {
		HashSection = (1 << HashSize - 3) - 1;
		HashSize = (1 << HashSize) - 2;
