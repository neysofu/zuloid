...
board_approves_pseudolegal_pawn_move(struct Board *board, Move move)
{
	...
	bool is_on_home_rank = coord_rank(source) == color_pawn_rank(active_color);
...
board_approves_pseudolegal_move(struct Board *board,
                                ...
                                enum Piece piece)
{
	enum Color source_color =
	  (board->bb_colors & bb_coord(move_source(move))) > COLOR_WHITE;
	enum Color target_color =
	  (board->bb_colors & bb_coord(move_target(move))) > COLOR_WHITE;
	...
	switch (piece) {
		...
		case PIECE_NONE:
			return false;
		default:
			assert(piece == PIECE_BISHOP || piece == PIECE_ROOK ||
			       piece == PIECE_QUEEN);
...
board_push_pseudolegal_move(struct Board *board, Move move, enum Piece piece)
{
	...
	bool is_king_capture =
	  board->bb_pieces[PIECE_KING] & bb_coord(move_target(move));
	if (is_king_capture) {
	...
	if (board_num_half_moves(board) >= BOARD_MAX_HALF_MOVES) {
		return (struct Result){
			.winner = COLOR_NONE, .termination = TERMINATION_NO_CAPTURE,
		};
	}
	return RESULT_NONE;
}
