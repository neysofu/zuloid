...
bool
move_is_reversible(const struct Move move, const struct Board *board) {
	return !(move_is_capture(move, board) ||
			 board_square(board, move.source)->piece == PIECE_PAWN);
}
...
board_push(struct Board *board, const struct Move move) {
	printf("%i\n", move_is_reversible(move, board));
	if (move_is_reversible(move, board)) {
